   ...
   return (
      <Transitioner single={true}>
         <div className="h-full w-full flex justify-center items-center bg-zinc-800">
            <div className="w-4/5 h-4/5 bg-slate-700 mt-24 flex items-center relative">
               ...
               <div className="toEdit h-4/5 w-auto text-slate-100 text-2xl absolute left-[580px] top-[290px]">
                  <div className="w-full flex my-5">
                     ...
                     <input type="text" onChange={changeBookInfo} defaultValue={bookInfo.book_name} className="book_name text-black text-xl w-[450px] p-2" />
                  </div>
                  <div className="w-full flex my-5">
                     <div className="cursive">New Number of Pages:&nbsp;&nbsp;</div>
                     <input type="text" onChange={changeBookInfo} defaultValue={bookInfo.pages} className="pages text-black text-xl w-[100px] p-2" />
                  </div>
                  <div className="w-full flex my-5">
                     <div className="cursive">New Number of Ratings:&nbsp;&nbsp;</div>
                     <input type="text" onChange={changeBookInfo} defaultValue={bookInfo.rating} className="rating text-black text-xl w-[100px] p-2" />
                  </div>
                  <button
                     disabled={changes === 0}
                     className="bg-cyan-600 rounded-full p-3 disabled:cursor-not-allowed disabled:hover:bg-cyan-600 hover:bg-cyan-700 transition-all duration-200 ease-in-out" onClick={saveChanges}>
                     Save Changes
                  </button>
               </div>
            </div>
